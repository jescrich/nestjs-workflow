...
### Added
- **Monorepo Structure**: Reorganized project into packages/base and packages/examples
  - Core library moved to `packages/base`
  - All examples consolidated in `packages/examples`
  - Improved project organization and maintainability
- **Example Applications**: Added 4 comprehensive example projects
  - 00-basic-example: Simple task workflow with BullMQ
  - 01-user-onboarding: Complete user onboarding workflow with verification
  - 02-order-processing: E-commerce order processing workflow
  - 03-kafka-inventory: Kafka-driven inventory management
  - 04-bullmq-task: BullMQ task management with interactive demo
- **Interactive Demos**: Each example includes runnable demos with workflow visualization
- **CI/CD Improvements**: 
  - Updated test configuration to skip BullMQ integration tests in CI (no Redis required)
  - Automatic GitHub release creation on NPM publish
  - Enhanced build pipeline configuration

### Changed
- Restructured repository to monorepo format for better organization
- Updated documentation to reflect new project structure
- Improved test infrastructure with conditional test execution

### Fixed
- CI pipeline now properly skips Redis-dependent tests
