...
import { EventMessage } from './kafka/event.handler';
    ...
    for (const bullmqDef of this.definition.bullmq.events) {
      await this.bullmqClient.consume(
        ...
        async (job) => {
          ...
          this.logger.log(
            `BullMQ job received on queue ${bullmqDef.queue} with job ID ${job.id} for URN ${urn}`,
            urn,
          );

          try {
            await this.emit({ event: bullmqDef.event, urn, payload: payload as T | P | object | string });
            this.logger.log(`BullMQ job processed successfully for job ID ${job.id}`, urn);
          } catch (error) {
            this.logger.error(
              `BullMQ job processing failed for job ID ${job.id}: ${error.message}`,
              urn,
            );
